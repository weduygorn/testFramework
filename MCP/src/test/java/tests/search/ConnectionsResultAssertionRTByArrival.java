...
public class ConnectionsResultAssertionRTByArrival extends BaseTest {

    @Test(groups = {"dev", "test", "demo"}, dataProvider = "RomaTermini_MilanoCentrale", dataProviderClass = TestDataProvider.class)
    public void connectionDetailsAssertionRTByArr(String dep, String arr) {
        gotoUrl(APP_URL);
        s_assert.assertTrue(searchPage.searchTitle.getText().equals("International booking"), "search title");
        SearchParams searchParams = setupSearchParamsWithRTArrival(dep, arr, "0000", "0000");
...
//        s_assert.assertTrue(connectionResultPage.connectionsBlock.stations.getText().contains(dep), "Roma Termini2");
//        s_assert.assertTrue(connectionResultPage.connectionsBlock.stations.getText().contains(arr), "Milano Centrale2");
//        s_assert.assertTrue(connectionResultPage.connectionsBlock.time.getText().contains(":"), "time");
//        s_assert.assertTrue(connectionResultPage.connectionsBlock.duration.getText().contains(":"), "duration");
//        s_assert.assertTrue(connectionResultPage.connectionsBlock.changes.getText().contains("0"), "changes");
//        s_assert.assertTrue(connectionResultPage.connectionsBlock.product.getText().contains("FR"), "product");
//
...
//
//        s_assert.assertTrue(connectionResultPage.getFirstConnection().toOfferSelection.getText().equals("Later"), "later");
//        connectionResultPage.connectionsBlock.toOfferSelection.getText().equals("Return journey");
//        connectionResultPage.clickToOfferSelectionBtn();
        ...
        s_assert.assertTrue( connectionResultPage.getFirstConnection().toOfferSelection.getText().equals("To offer selection"), "later");
